...
export default function ListTodo({title,description,start_time,end_time,status}:attributeListType) {
    return (
        <Box className='w-[334px] bg-white p-5 border-2 rounded-[8px] py-[18px] px-[24px]'>
            <VStack>
                ...
                <HStack className='items-center gap-3 mt-3'>
                    <ClockFadingIcon />
                    <Text className='text-[14px]' style={{ fontFamily: "HankenGrotesk_500Medium_Italic" }}>{`WIB: ${start_time} - ${end_time}`}</Text>
                </HStack>
            </VStack>
            <View className={`border-2 rounded-br-[6px] rounded-tl-[6px] absolute right-[-1px] ${status === 'On Progress' ? 'bg-[#D9A127]':'bg-[#6BBC1A]'} bottom-0 px-4 mt-2 py-[8px]`}>
